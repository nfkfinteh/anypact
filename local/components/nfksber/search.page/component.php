...
if($this->InitComponentTemplate($templatePage))
{
	...
	if(strlen($folderPath) > 0)
	{
		...
		if($obSearch->errorno==0)
		{
			...
			//-------------- Кастом start --------------
			if (empty($ar)) {
				$arSearchQuery = explode(" ", $arFilter['QUERY']);
				$maxItemsStr = array(
					'STR' => '',
					'COUNT' => 0,
				);
				foreach ($arSearchQuery as $SearchQuery) {
					$obSearch = new CSearch();
					$arFilterNewSearch = $arFilter;
					$arFilterNewSearch['QUERY'] = $SearchQuery;
					$obSearch->Search($arFilterNewSearch, $aSort, $exFILTER);
					$cnt = 0;
					while ($arSearchRes = $obSearch->Fetch()) {
						$cnt++;
				   	}
				   	if ($maxItemsStr['COUNT'] < $cnt) {
				   		$maxItemsStr = array(
							'STR' => $SearchQuery,
							'COUNT' => $cnt,
						);
				...
				if ($maxItemsStr['COUNT'] > 0) {
					$arResult['ORIGINAL_SEARCH_QUERY_STR'] = $arFilter['QUERY'];
					$arResult['REPLACE_SEARCH_QUERY_STR'] = $arFilter['QUERY'] = $maxItemsStr['STR'];
					$obSearch = new CSearch();
					$obSearch->SetOptions(array(
						"ERROR_ON_EMPTY_STEM" => $arParams["RESTART"] != "Y",
						"NO_WORD_LOGIC" => $arParams["NO_WORD_LOGIC"] == "Y",
