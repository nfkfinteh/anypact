...
$itemSize = count($arResult['SECTION']);
for($index = 0; $index < $itemSize; $index++)
{
	$title = htmlspecialcharsex($arResult['SECTION'][$index]["NAME"]);
	...
	if($index != $itemSize-1)
	{
		$strReturn .= '
			<div class="bx-breadcrumb-item" id="bx_breadcrumb_'.$index.'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
				'.$arrow.'
				<a href="/pacts/?SECTION_ID='.$arResult['SECTION'][$index]["ID"].'" title="'.$title.'" itemprop="item">
