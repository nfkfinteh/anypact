...
$APPLICATION->SetTitle(GetMessage("moderation_edit_deal"));
...
<form method="POST" action="<?= $APPLICATION->GetCurPage()?>?lang=<?= LANGUAGE_ID?>" name="moderation_edit_deal">
...
<?
$aTabs = array(
	array("DIV" => "moderation_edit_deal", "TAB" => GetMessage("MODERN_TITLE"), "ICON" => "gosuslugi", "TITLE" => GetMessage("moderation_edit_deal"))
);

if(CModule::IncludeModule('iblock')){
	$rsData = CIBlockElement::GetList(array('id' => 'desc'), array('IBLOCK_ID' => 10), false, false, array('ID', 'NAME', 'CREATED_BY', "PROPERTY_ORIGINAL_DEAL"));
					...
					<table class="adm-list-table" id="tbl_perfmon_table_moderation_deal">
						<thead>
							<tr class="adm-list-table-header">
								...
								<td class="adm-list-table-cell">
									<div class="adm-list-table-cell-inner"><?=GetMessage("moderation_deal_DEAL_ORIGINAL")?></div>
								</td>
								<td class="adm-list-table-cell">
									<div class="adm-list-table-cell-inner"><?=GetMessage("moderation_deal_DEAL_EDIT")?></div>
								</td>
								<td class="adm-list-table-cell">
...
<script>
	$('.mod_success').on('click', function(e) {
		...
		if (confirm("<?=GetMessage('moderation_deal_DEAL_ACCEPT_WARNING')?>")) {
			$.ajax({
				type: "POST",
				url: '/response/ajax/moderation_edit_deal.php',
				data: {
					id: id,
					action: 'accept',
