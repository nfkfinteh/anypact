...
if ( empty($EmailValue) )
{
    $EmailValue = ArrayHelper::GetValuePath( $this->Preview, 'open_smev/form_email' );
}
...
if ( empty($PhoneValue) )
{
    $PhoneValue = ArrayHelper::GetValuePath( $this->Preview, 'open_smev/form_phone' );
}

//$PhoneValue = PhoneHelper::FormatPhone( $PhoneValue, '7($1) $2-$3-$4' );
...
$PASSAIR = "js-air-datepicker='" . json_encode( ['maxDate' => $CurrentDateTime->format('d.m.Y')] ) . "'";
...
При возникновении вопросов обратитесь по телефону 8 800 200-84-84</div>
		 ...
		 <div class='colwpl col-xs-4'><label for='last_name'>Фамилия <span></span></label><br/><input js-smev-input type='text' class='form-control' id='last_name' name='last_name' value='".$arResult['last_name']."'></div>
		 <div class='colwpl col-xs-4'><label for='first_name'>Имя <span></span></label><br/><input js-smev-input type='text' class='form-control' id='first_name' name='first_name' value='".$arResult['first_name']."'></div>
		 <div class='colwpl col-xs-4'><label for='second_name'>Отчество <span></span></label><br/><input js-smev-input type='text' class='form-control' id='second_name' name='second_name' value='".$arResult['second_name']."'></div>
         <div class='clearfix'></div>
		 
		 <div class='colwpl col-xs-12'><label for='birth_place'>Место рождения <span></span></label><br/><input type='text' class='form-control' id='birth_place' name='birth_place' value='".$arResult['birth_place']."' ></div>
		 <div class='colwpl col-xs-4'><label for='birth_day'>Дата рождения <span></span></label><br/><input type='text' class='form-control' id='birth_day' name='birth_day' value='".$arResult['birth_day']."' ".$DBAIR." ></div>
		 <div class='colwpl col-xs-4'><label for='citizen'>Гражданство <span></span></label><br/>
		 <select class='form-control' id='citizen' name='citizen'>
			 <option value='RUS' selected>Российская Федерация</option>
...
$out .= "<div class='colwpp col-xs-12'><h4>Контактные данные</h4>
		...
		<div class='colwpl col-xs-4'><label for='phone'>Мобильный телефон <span></span></label><br/><input type='text' class='form-control' id='phone' name='phone' value='".$PhoneValue."' xxx js-value='".$PhoneValue."' yyy></div>
		</div><div class='clearfix'></div>";

$out .= "<div class='colwpp col-xs-12'><h4>Документ</h4>
		<div class='colwpl col-xs-12 col-md-3'><label for='type_doc'>Вид документа <span></span></label><br/>
		
		<select class='form-control' id='type_doc' name='type_doc'>
			 <option value='RF_PASSPORT' selected>Российский паспорт</option>
			 <option value='UA_PASSPORT'>Украинский паспорт</option>
		    
		</select>
		
		</div>
		<div class='colwpl col-xs-2'><label for='pass_seria'>Серия <span></span></label><br/><input js-smev-input type='text' class='form-control' id='pass_seria' name='pass_seria' value='".$arResult['pass_seria']."' ></div>
		<div class='colwpl col-xs-2'><label for='pass_number'>Номер <span></span></label><br/><input js-smev-input type='text' class='form-control' id='pass_number' name='pass_number' value='".$arResult['pass_number']."' ></div>
		<div class='colwpl col-xs-2 col-md-3'><label for='pass_dv'>Дата выдачи <span></span></label><br/><input type='text' class='form-control' id='pass_dv' name='pass_dv' value='".$arResult['pass_dv']."' ".$PASSAIR."></div>
		<div class='colwpl col-xs-2'><label for='pass_kp'>Код подразделения <span></span></label><br/><input type='text' class='form-control' id='pass_kp' name='pass_kp' value='".$arResult['pass_kp']."' ></div>
		</div><div class='clearfix'></div>
		<div class='colwpp col-xs-12'><div class='colwpl col-xs-12'><label for='pass_who'>Кем выдан <span></span></label><br/><input type='text' class='form-control' id='pass_who' name='pass_who' value='".$arResult['pass_who']."' ></div></div>
		";

$out .= "<div class='clearfix'></div><div class='colwpp col-xs-12'><div class='colwpl col-xs-4'><label for='inn'>СНИЛС <span></span></label><br/>
<input js-smev-input type='text' class='form-control' id='snils' name='snils' maxLength='32' value='".$arResult['snils']."' js-value=".$arResult['snils']." ></div></div><div class='clearfix'></div>";

$out .= "<div class='clearfix'></div><div class='colwpp col-xs-12'><div class='colwpl col-xs-4'><label for='inn'>ИНН <span></span></label><br/>
<input js-smev-input type='text' class='form-control' id='inn' name='inn' maxLength='12' value=".$arResult['inn']." ></div></div><div class='clearfix'></div>";

ob_start();

require_once $this->DIRRECTORY_ESIA_TPL . 'smev/check.php';

$out .= ob_get_contents();

ob_end_clean();

$out .= "<div js-smev-hidden style='display: none;'><div class='colwpp col-xs-12 address_information'><h4>Адрес регистрации</h4>Данные вводятся в соответствии с паспортом гражданина РФ.<br/>
...
Если в Вашем почтовом адресе отсутствует название района, улицы, корпуса и др., нажмите на кнопку «нет» рядом с соответствующем полем.<br/><br/>
        ...
        <div class='colwpl col-xs-4'><label for='country'>Страна<br><span></span></label><br/><input type='text' class='form-control' size='61' name='country_post' maxLength=80 value='Российская Федерация' disabled></div>
		<div class='colwpl col-xs-7'><label for='oblast'>Республика, край, область, автономный округ, город федерального значения<span></span></label><input type='text' class='form-control' id='oblast_post' size='61' name='oblast_post' value='" . ArrayHelper::Value( $this->Current, 'oblast_post', ArrayHelper::Value( $this->Current, 'oblast' ) ) . "' maxLength=80 placeholder='название' js-federal-city='#city_post'></div>
